...
class Services extends MY_Controller {
	...
	public function __construct()
	{
		...
		$this->data['breadcrumb'] = '
			...
			<li><a href="'.base_url($this->data['controller']).'">Daftar Layanan</a></li>
		';
	}
	...
	public function create()
	{
		if($this->data['user_access_create']==true){
			if($this->input->server('REQUEST_METHOD')=='POST'){
				...
				$this->call_sp("ins_SERVICES", $params);
	...
	public function edit()
	{
		if($this->data['user_access_update']==true){
			if($this->input->server('REQUEST_METHOD')=='POST'){
				...
				$this->call_sp("upd_SERVICES", $params);
				if(sizeof($this->error)==0){
				...
				$this->data['edit'] = $this->call_sp("sel_SERVICES", array(0,0,$id,""));
		...
		if($this->data['user_access_delete']==true){
			if($this->input->server('REQUEST_METHOD')=='POST'){
				...
				foreach($data['DEL'] as $del){
					$this->call_sp("del_SERVICES", array($del));
				}
			}else{
