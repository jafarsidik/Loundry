...
class Customer_types extends MY_Controller {
	...
	public function __construct()
	{
		...
		$this->data['breadcrumb'] = '
			...
			<li><a href="'.base_url($this->data['controller']).'">Jenis Pelanggan</a></li>
		';
	}
	...
	public function index($offset=0)
	{
		if($this->data['user_access_read']==true){
			...
			$pagination = array(
				...
				'total_rows' => sizeof($this->call_sp("sel_CUSTOMER_TYPES", $params)),
				'per_page' => $perpage,
				'num_links' => 3,
				'uri_segment' => 3
			);
			$params = array($perpage, $offset, "", (isset($this->data['search']['SEARCH_BY']) ? $this->data['search']['SEARCH_BY'] : ""));
			$this->data['lists'] = $this->call_sp("sel_CUSTOMER_TYPES", $params);
	...
	public function create()
	{
		if($this->data['user_access_create']==true){
			if($this->input->server('REQUEST_METHOD')=='POST'){
				...
				$this->call_sp("ins_CUSTOMER_TYPES", $params);
	...
	public function edit()
	{
		if($this->data['user_access_update']==true){
			if($this->input->server('REQUEST_METHOD')=='POST'){
				...
				$this->call_sp("upd_CUSTOMER_TYPES", $params);
				if(sizeof($this->error)==0){
				...
				$this->data['edit'] = $this->call_sp("sel_CUSTOMER_TYPES", array(0,0,$id,""));
		...
		if($this->data['user_access_delete']==true){
			if($this->input->server('REQUEST_METHOD')=='POST'){
				...
				foreach($data['DEL'] as $del){
					$this->call_sp("del_CUSTOMER_TYPES", array($del));
				}
			}else{
