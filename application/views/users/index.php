<?= $breadcrumb ?>
<h2 class="page-header">Pengguna</h2>
<div class="page-wrapper">
	<form action="<?= base_url($controller) ?>" method="post">
	<div class="page-search">
		<div class="row">
			<div class="col-md-12">
				<input type="text" class="form-control search_typing" name="<?= $controller ?>[SEARCH_BY]" value="<?= (isset($search) ? $search['SEARCH_BY'] : "") ?>" maxlength="50" placeholder="Cari : Group, Nama" autofocus>
			</div>
		</div>
	</div>
		...
		<tbody>
			<?php
				...
				foreach($lists as $l){
					...
					echo '<td>';
						echo '<a href="'.base_url($controller.'/edit/'.$l->USER_ID).'" class="tooltips" title="Ubah"><i class="fa fa-edit fa-lg"></i></a> ';
						echo '<a href="'.base_url($controller.'/delete/'.$l->USER_ID).'" class="tooltips" title="Hapus"><i class="fa fa-trash-o fa-lg"></i></a> ';
						echo '<a href="javascript:void(0)" class="chkbox tooltips" title="Pilih"><label><input type="checkbox" class="hidden" name="'.$controller.'[DEL][]" value="'.$l->USER_ID.'"><span class="fa fa-lg"></span></label></a> ';
					echo '</td>';
					echo '</tr>';
					$num += 1;
				}
