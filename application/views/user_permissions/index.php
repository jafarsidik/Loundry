<?= $breadcrumb ?>
<h2 class="page-header">Group Pengguna</h2>
<div class="page-wrapper">
	<form action="<?= base_url($controller) ?>" method="post">
	<div class="page-search">
		<div class="row">
			<div class="col-md-12">
				<input type="text" class="form-control search_typing" name="<?= $controller ?>[SEARCH_BY]" value="<?= (isset($search) ? $search['SEARCH_BY'] : "") ?>" maxlength="50" placeholder="Cari : Nama" autofocus>
			</div>
		</div>
	</div>
		...
		<tbody>
			<?php
				...
				foreach($lists as $l){
					echo '<tr>';
					echo '<td>'.$num.'</td>';
					echo '<td>'.$l->PERMISSION_NAME.'</td>';
					echo '<td>'.$l->DESCRIPTION.'</td>';
					echo '<td>';
						echo '<a href="'.base_url($controller.'/edit/'.$l->PERMISSION_ID).'" class="tooltips" title="Ubah"><i class="fa fa-edit fa-lg"></i></a> ';
						echo '<a href="'.base_url($controller.'/delete/'.$l->PERMISSION_ID).'" class="tooltips" title="Hapus"><i class="fa fa-trash-o fa-lg"></i></a> ';
						echo '<a href="javascript:void(0)" class="chkbox tooltips" title="Pilih"><label><input type="checkbox" class="hidden" name="'.$controller.'[DEL][]" value="'.$l->PERMISSION_ID.'"><span class="fa fa-lg"></span></label></a> ';
					echo '</td>';
					echo '</tr>';
					$num += 1;
				}
